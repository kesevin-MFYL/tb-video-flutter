import 'package:editvideo/config/log/logger.dart';
import 'package:editvideo/manager/remote_config_manager.dart';
import 'package:flutter/material.dart';
import 'package:google_mobile_ads/google_mobile_ads.dart';

/// 激励广告（Rewarded Ad）管理器
///
/// 职责：
/// - 负责通过 AdMob SDK 实际加载和展示激励视频类型的广告。
/// - 管理不同场景（scenario）下的广告实例和缓存状态。
/// - 当加载失败或被关闭时，与外层的 [AdManager] 联动。
class RewardAdManager {
  /// 单例模式，方便全局统一管理多场景激励广告
  static final RewardAdManager instance = RewardAdManager._internal();
  factory RewardAdManager() => instance;
  RewardAdManager._internal();

  /// 激励广告缓存的有效时长（当前业务设置为 1 小时过期）
  final Duration maxCacheDuration = const Duration(hours: 1);

  /// 缓存每个场景的广告加载时间，用于判断是否过期
  final Map<String, DateTime> _rewardedLoadTimes = {};
  
  /// 缓存每个场景加载成功的广告实例
  final Map<String, RewardedAd> _rewardedAds = {};
  
  /// 记录每个场景是否正在加载中，防止重复发起请求
  final Map<String, bool> _isAdLoadingMap = {};
  
  /// 标记当前是否正在展示广告，防止重复展示
  bool _isShowingAd = false;

  /// 尝试加载单个广告配置项
  ///
  /// 由统一的 [AdManager] 调度调用。如果当前加载失败，会触发 [onFailed] 回调，
  /// 通知 [AdManager] 继续尝试下一个优先级的配置项。
  void loadAdItem(String scenario, AdItem item, {required VoidCallback onFailed, required VoidCallback onSuccess}) async {
    if (isAdLoading(scenario)) {
      commonDebugPrint('RewardAdManager: RewardedAd for scenario $scenario is already loading. Ignored duplicate request.');
      return;
    }

    _isAdLoadingMap[scenario] = true;

    RewardedAd.load(
      adUnitId: item.placementid,
      request: const AdRequest(),
      rewardedAdLoadCallback: RewardedAdLoadCallback(
        onAdLoaded: (ad) {
          commonDebugPrint('测试日志：场景$scenario--拉取广告成功-广告类型:${item.adtype}--广告id: ${item.placementid}--优先级：${item.adweight}');
          commonDebugPrint('RewardAdManager: RewardedAd ${item.placementid} loaded for scenario: $scenario');
          // 加载成功，记录加载时间和广告实例
          _rewardedLoadTimes[scenario] = DateTime.now();
          _rewardedAds[scenario] = ad;
          _isAdLoadingMap[scenario] = false;
          onSuccess(); // 通知调度器该场景加载流程已成功闭环
        },
        onAdFailedToLoad: (error) {
          commonDebugPrint('RewardAdManager: RewardedAd ${item.placementid} failed to load for scenario $scenario: $error');
          _isAdLoadingMap[scenario] = false;
          // 加载失败，通知调度器继续下一个
          onFailed();
        },
      ),
    );
  }

  /// 检查指定场景的激励广告是否已加载并可用
  bool isAdAvailable(String scenario) {
    return _rewardedAds.containsKey(scenario) && _rewardedAds[scenario] != null;
  }

  /// 检查指定场景的激励广告是否正在加载中
  bool isAdLoading(String scenario) {
    return _isAdLoadingMap[scenario] ?? false;
  }

  /// 展示指定场景的广告
  ///
  /// [onAdDismissed]：由 [AdManager] 传入的回调，用于在广告被关闭或因过期丢弃时触发重新加载逻辑。
  void showAdIfAvailable(String scenario, {ValueChanged<bool>? onAdDismissed, VoidCallback? onUserEarnedReward}) {
    if (!isAdAvailable(scenario)) {
      commonDebugPrint('RewardAdManager: Tried to show ad before available for scenario: $scenario.');
      if (onAdDismissed != null) onAdDismissed(false);
      return;
    }
    if (_isShowingAd) {
      commonDebugPrint('RewardAdManager: Tried to show ad while already showing an ad.');
      return;
    }

    // 检查广告是否已经过期（超过 1 小时）
    final loadTime = _rewardedLoadTimes[scenario];
    if (loadTime != null && DateTime.now().subtract(maxCacheDuration).isAfter(loadTime)) {
      commonDebugPrint('RewardAdManager: Maximum cache duration exceeded for scenario: $scenario. Loading another ad.');
      disposeAd(scenario);
      if (onAdDismissed != null) onAdDismissed(false);
      return;
    }

    // 取出对应场景的广告实例
    final ad = _rewardedAds[scenario]!;
    
    // 设置全屏内容的回调监听
    ad.fullScreenContentCallback = FullScreenContentCallback(
      onAdShowedFullScreenContent: (ad) {
        commonDebugPrint('RewardAdManager: $ad onAdShowedFullScreenContent for scenario: $scenario');
        _isShowingAd = true;
      },
      onAdFailedToShowFullScreenContent: (ad, error) {
        commonDebugPrint('RewardAdManager: $ad onAdFailedToShowFullScreenContent for scenario $scenario: $error');
        _isShowingAd = false;
        disposeAd(scenario); // 展示失败时销毁并清理缓存
      },
      onAdDismissedFullScreenContent: (ad) {
        commonDebugPrint('RewardAdManager: $ad onAdDismissedFullScreenContent for scenario: $scenario');
        _isShowingAd = false;
        disposeAd(scenario); // 用户关闭广告后销毁并清理缓存

        if (onAdDismissed != null) {
          Future.delayed(const Duration(milliseconds: 100), () {
            onAdDismissed(true); // 触发回调通知调度器重新拉取备用广告
          });
        }
      },
    );
    
    // 实际调用 AdMob 的 show 方法，并且传入奖励回调
    ad.show(onUserEarnedReward: (AdWithoutView ad, RewardItem rewardItem) {
      commonDebugPrint('RewardAdManager: User earned reward ${rewardItem.amount} ${rewardItem.type} for scenario: $scenario');
      if (onUserEarnedReward != null) {
        onUserEarnedReward();
      }
    });
  }

  /// 销毁并清理指定场景的广告实例及其状态
  void disposeAd(String scenario) {
    _rewardedAds[scenario]?.dispose();
    _rewardedAds.remove(scenario);
    _rewardedLoadTimes.remove(scenario);
  }
}
